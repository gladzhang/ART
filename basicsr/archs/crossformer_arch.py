#Apply the model design of CrossFormer to train for image SR
import math
import torch
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        assert C % 2 == 0, f"Channel size is not even"
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, embed_dim=96, norm_layer=None):
        ...
        patches_resolution = [img_size[0], img_size[1]]  # only for flops calculation
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[1, 3], in_chans=3,
                 embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24),
                 group_size=7, crs_interval=(8, 4, 2, 1), mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True, upscale=2, img_range=1.,
                 use_checkpoint=False, merge_size=[[1, 3], [1, 3], [1, 3]], **kwargs):
        super().__init__()
...
if __name__ == '__main__':
    ...
    width = 128
    model = CrossFormer(
        img_size=64, patch_size=[1, 3], in_chans=3,
        embed_dim=180, depths=(6, 6, 6, 6, 6, 6), num_heads=(6, 6, 6, 6, 6, 6),
        group_size=8, crs_interval=(4, 4, 4, 4, 4, 4), mlp_ratio=4., qkv_bias=True, qk_scale=None,
        drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        norm_layer=nn.LayerNorm, patch_norm=True, upscale=2, img_range=1.,
        use_checkpoint=False, merge_size=[[1, 3], [1, 3], [1, 3], [1, 3], [1, 3]]
    )
    # print(model)
